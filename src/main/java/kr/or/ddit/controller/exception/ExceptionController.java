...
public class ExceptionController {
	 ...
	 *  	
	 *  	2-1) 예외 발생 상황
	 *  	- 등록 할 때 제목에 빈 값을 입력하여 유효값 검증 예외 발생
	 *  		> Http Status 400
	 *  		> [예외 상황 만들기]
	 ...
	 *  		> Http Status 500
	 *  		> [예외 상황 만들기]
	 *  			sqlmap/boardMapper_SQL.xml의 id가 'delete' 쿼리에서 board_no를 board_no2로 수정
	 *  		> 요청URL
	 ...
	 *  - 웹 컨테이너 설정 파일(web.xml)의 <error-code> 요소에 상태 코드를 설정하고 <location> 요소에 이동 대상 페이지를 지정
	 *  
	 *  	예외 처리 방법
	 *  	- 웹 컨테이너 설정(web.xml)
	 ...
	 *  		> 존재 하지 않는 페이지 URL 요청시 예외 발생
	 *  
	 *  	- 예외 처리 결과
	 *  		> 등록 할 때 제목에 빈 값을 입력하여 유효값 검증 예외 발생		:: 처리완료
	 *  		> 수정 화면 생성 할 때 뷰 파일에서 예외 발생					:: 처리완료
	 *  		> 삭제 할 때 매핑 파일에서 예외 발생						:: 처리완료
	 *  		> 존재 하지 않는 게시물을 조회할 때 사용자가 정의한 예외 발생		:: 처리완료
	 *  		> 존재 하지 않는 페이지 URL 요청시 예외 발생 					:: 처리완료
	 *  
	 ...
	 *  		> Controller에서 선언한 URL 매핑 정보로 설정 가능
	 *  		> 서블릿 3.1이상 지원
	 *  
	 *  	- 처리 할 수 있는 예외
	 *  		> 등록 할 때 제목에 빈 값을 입력하여 유효값 검증 예외 발생
	 ...
	 *  		> 입력값 검증 대상인 자바빈즈 메서드 매개변수에 @Validated를 지정하고 바로 다음에 BindingResult를 정의
